...
class LatestNews extends StatelessWidget {
  ...
  Future<List<dynamic>> fetchLatestNewsletter() async {
    try {
      final url = Uri.parse(
          'https://thehive-api.up.railway.app/api/v1/articles/newsletter/latest');
      final response = await http.get(url);

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final sectionList = data['data'] ??
            []; // Usamos un valor vacío por si no existe o está vacío
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, int index) {
            ...
            return Padding(
              ...
              child: NewsCard(
                imageUrl:
                    newsItem['image_url'] ?? 'https://via.placeholder.com/150',
